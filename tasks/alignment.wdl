...
task RealignBam {
  input {
    ...
    # Disk size
    Float bam_size = select_first([size(bam, "GB"), 1.0])
    Int? bam_disk_size = ceil(select_first([bam_size, 2.0]) * 4)
    Int job_disk_size = select_first([disk_size, bam_disk_size, 8])
  }
  ...
  command <<<
    ...
    samtools collate -@ $nt -Ouf "~{bam}" | \
      samtools fastq -@ $nt -t -0 /dev/null - | \
      sentieon bwa mem -t $nt ~{bwa_xargs} -K ~{bwa_karg} -p -C \
      -H rg_header.txt "~{ref_fasta}" /dev/stdin | \
      sentieon util sort -t $nt --sam2bam -o "~{sample_name}_sorted.bam" -i - ~{sort_xargs}
  >>>
  runtime {
    preemptible: preemptible_tries
    docker: sentieon_docker
  ...
  output {
    # Alignment files
    File aligned_reads = "~{sample_name}_sorted.bam"
    File aligned_index = "~{sample_name}_sorted.bam.bai"
  }
}
